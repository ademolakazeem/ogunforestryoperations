...
//page permission id = 7, permission name =  view current accounts
...
<?php 
	...
	//pagination
		...
		if($acc_level == 2)
		{
			$full_sql = "select * from tbl_contractor_account WHERE contractor_id in (select id from tbl_contractor WHERE business_area = '$location')order by contractor_id, created_date desc"; 
		...
		{//hqusers with acc level = 3 should not see information about Area J4 Corporation with location id = 10
			$full_sql = "select * from tbl_contractor_account WHERE contractor_id in (select id from tbl_contractor WHERE business_area NOT IN (SELECT forest_location from tbl_location WHERE id = 10))order by contractor_id, created_date desc"; 
			//full sql before split in to pages
		...
		elseif($acc_level == 4)
		{
			...
			$full_sql = "select * from tbl_contractor_account WHERE contractor_id in (select id from tbl_contractor WHERE business_area IN (SELECT forest_location from tbl_location WHERE id = 10))order by contractor_id, created_date desc"; 
			//full sql before split in to pages
		...
		{
			$full_sql = "select * from tbl_contractor_account order by contractor_id, created_date desc"; 
			//full sql before split in to pages
...
if (isset($_POST['search']))
	...
	//pagination
		...
		if($acc_level == 2)
		{
			$full_sql = "select * from tbl_contractor_account where contractor_id ='$keyword' && contractor_id in (select id from tbl_contractor WHERE business_area = '$location')";
			$full_sql_count = "select sum(amount_deposited)amount from tbl_contractor_account where contractor_id ='$keyword' && contractor id in (select id from tbl_contractor WHERE business_area = '$location')";
		}
		elseif($acc_level == 3)
		{//hqusers with acc level = 3 should not see information about Area J4 Corporation with location id = 10
			$full_sql = "select * from tbl_contractor_account where contractor_id ='$keyword' && contractor_id in (select id from tbl_contractor WHERE business_area NOT IN (SELECT forest_location from tbl_location WHERE id = 10))";
			$full_sql_count = "select sum(amount_deposited)amount from tbl_contractor_account where contractor_id ='$keyword' && contractor id in (select id from tbl_contractor WHERE business_area NOT IN (SELECT forest_location from tbl_location WHERE id = 10))";
		}
		else
		{
		$full_sql = "select * from tbl_contractor_account where contractor_id ='$keyword'";
...
<div class="content">
						<h4>
					      ...
					      <br>
				        Total Existing Contractors: <?php echo $pageObj->getTotalRow(); ?> &nbsp;&nbsp;&nbsp;&nbsp;<a href="_new_contractor_account.php" title="Create New Contractor" class="green">Create New Contractor Account</a>
                    ...
                    <table width="649" id="background-image" summary="Meeting Results">
                      <thead>
                        <tr>
                          ...
                          <th width="41" align="center" scope="col"><strong>Created Date</strong></th>

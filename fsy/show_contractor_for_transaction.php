...
<?php 
	...
	//pagination
		/*** Variables ***/
		$page = 1; //default page
		$per_page = 1000000; //rows per page
...
if (isset($_POST['search']))
	...
	//pagination
		...
		if($acc_level == 2)
		{		
			$full_sql = "select * from tbl_contractor where id='$keyword' && business_area = '$location' order by datereg desc";
		}
		elseif($acc_level == 3)
		{	//hqusers with acc level = 3 should not see information about Area J4 Corporation with location id = 10
			$full_sql = "select * from tbl_contractor where id='$keyword' && business_area NOT IN (SELECT forest_location from tbl_location WHERE id = 10) order by datereg desc";
		}
		else
		{$full_sql = "select * from tbl_contractor where id='$keyword' order by datereg desc";}		
		$display_links = 11; //number of links to be displayed - odd number
...
<div class="content">
						<h4>
					      ...
					      <br>
				        Total Existing Contractor: <?php echo $pageObj->getTotalRow(); ?> &nbsp;&nbsp;&nbsp;&nbsp;<a href="_registration.php" title="Register new staff" class="green">Create New Contractor</a></h4>
						<p>&nbsp;</p>
...
<p>&nbsp; </p>
                    ...
                    <table width="649" id="background-image" summary="Meeting Results">
                      <thead>
                        <tr>
                          ...
                          <th width="172" align="center" scope="col"><span class="td_ele"><strong>New Transaction</strong></span></th>
                        </tr>
                      </thead>
                      <tfoot>
                        <tr>
 ...
 <td align="center"><a href="_new_contractor_transaction.php?con_id=<?php echo $rs['id'];?>" title="Administer <?php echo $rs['name'];?>"><img src="../images/newaccount.png" width="27" height="25"></a></td>
                        </tr>
                        <?php $i++;}?>
                      </tbody>
                      <tfoot>
			...
			<div id="location">
			  <h4 class="footer-header white">Our <strong>Location</strong></h4>
				<img src="../images/map.png" alt="map" />
						<?php 
	require_once('commisioner_details.php');
	?>
			</div><!-- end location -->
		</div><!-- end widget -->
	</div> <!--! end widget-wrap -->

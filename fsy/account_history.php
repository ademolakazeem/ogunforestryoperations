...
<?php 
	...
	else{$qry = "SELECT * FROM tbl_contractor_account WHERE contractor_id = '$con_id'";}
	$rs = $db->fetchData($qry);
	...
	//pagination
		...
		if($acc_level == 2)
		{$full_sql = "select * from tbl_contractor_account_history where contractor_id='$con_id' && contractor_id in (select id from tbl_contractor WHERE business_area='$location') order by created_date desc";}
		elseif($acc_level == 3)//hqusers with acc level = 3 should not see information about Area J4 Corporation with location id = 10
		{$full_sql = "select * from tbl_contractor_account_history where contractor_id='$con_id' && contractor_id in (select id from tbl_contractor WHERE business_area NOT IN (SELECT forest_location from tbl_location WHERE id = 10)) order by created_date desc";}
		else
		{$full_sql = "select * from tbl_contractor_account_history where contractor_id='$con_id' order by created_date desc";}
...
if (isset($_POST['search']))
	...
	//pagination
		...
		if($acc_level == 2)
		{$full_sql = "select * from tbl_contractor_account_history where contractor_id ='$con_id' && teller_number='$keyword' && contractor_id  in (select id from tbl_contractor WHERE business_area = '$location')";}
		elseif($acc_level == 3)//hqusers with acc level = 3 should not see information about Area J4 Corporation with location id = 10
		{$full_sql = "select * from tbl_contractor_account_history where contractor_id ='$con_id' && teller_number='$keyword' && contractor_id  in (select id from tbl_contractor WHERE  business_area NOT IN (SELECT forest_location from tbl_location WHERE id = 10))";}
		else{$full_sql = "select * from tbl_contractor_account_history where contractor_id ='$con_id' and teller_number='$keyword'";}
...
<div class="content">
						<h4>
					      ...
					      <br>
				        Total Existing Contractor: <?php echo $pageObj->getTotalRow(); ?> &nbsp;&nbsp;&nbsp;&nbsp;<a href="_new_contractor_account.php" title="Create New Contractor" class="green">Create New Contractor Account</a></h4>
  ...
  <tr>
    <td width="158" height="26" align="left" valign="top"> Teller Number:</td>
    <td width="110"><p>
      <input type="text" name="keyword" id="keyword" size="20"/>
    </td>
    <td width="111"><input type="submit" value="Search" name="search2" id="search"/></td>
  </tr>
</table>
</div>
</form>
